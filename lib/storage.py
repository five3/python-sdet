...
class MysqlStorage(BaseStorage):
    ...
    def get(self, k, d=None):
        sql = '''select code, headers, `data`, no_pattern_response, data_type from http_mock where `key`=:key'''
    ...
    def set(self, k, v):
        sql = '''insert into http_mock (`key`, code, headers, `data`, no_pattern_response, data_type) 
                values (:key, :code, :headers, :data, :no_pattern_response, :data_type)
                ON DUPLICATE KEY UPDATE code=:code, headers=:headers, `data`=:data, 
                no_pattern_response=:no_pattern_response, data_type=:data_type'''
    ...
    def __str__(self):
        sql = '''select `key`, code, headers, `data`, no_pattern_response, data_type from http_mock'''
